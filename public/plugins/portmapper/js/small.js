function portmapper(moduleId) {
	...
	this.port2Html = function(mapping) {
		...
		if (!mapping.forced) {
			html.push('&nbsp;&nbsp;');
			html.push('<button class="btn btn-success btn-xs mapper', this.moduleId, '-save-port" data="', mapping.externalPort, '|', mapping.protocol, '|', mapping.internalIp, '|', mapping.name,
